############################################################
# Functions for the M2a model from codeml.
# 03.2021
############################################################
...
def parse(indir, features, outfile, pad):
    ...
    if features:
        headers = ["file","id","chr","start","end","lnl","k","num ps sites","clade","clade dn/ds","clade dn","clade ds"];
    else:
        headers = ["file","lnl","k","num ps sites","clade","clade dn/ds","clade dn","clade ds"];
    ...
    for d in os.listdir(indir):
        ...
        if features:
            ...
            gene_info = { 'id' : fid, 'chr' : cur_feature['chrome'], 'start' : cur_feature['start'], 'end' : cur_feature['end'],
                'lnl' : "NA", 'k' : "NA", 'dnds' : "NA", 'dn avg' : "NA", 'ds avg' : "NA", 'num ps sites' : 0 };
        else:
            gene_info = { 'lnl' : "NA", 'k' : "NA", 'dnds' : "NA", 'dn avg' : "NA", 'ds avg' : "NA", 'num ps sites' : 0 };
        ...
        for line in open(cur_codeml_file):
            ...
            if read_sites:
                ...
                if line.startswith("The grid"):
                    read_sites = False;
                continue;
            # Count the number of significant sites in this gene.

        gene_info['num ps sites'] = str(gene_info['num ps sites']);
